...
const Sidebar = ({ expand, setExpand }: SidebarProps): JSX.Element => {
    ...
    const [openMenu, setOpenMenu] = useState<{ id: string, open: boolean }>({ id: '0', open: false });
    ...
    return (
        <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
            <div>
                <div className={`flex ${expand ? 'flex-row gap-10' : 'flex-col items-center gap-8'}`}>
                    ...
                    <div onClick={() => expand ? setExpand(false) : setExpand(true)} className='group relative flex items-center justify-center hover:bg-gray-500/20 transition-all duration-300 h-9 w-9 aspect-square rounded-lg cursor-pointer'>
                        <Image 
                            className='md:hidden' 
                            src={assets.hamburger_sidebar_close} alt=''
                        />
                        <Image 
                            className='hidden md:block w-7' 
                            src={expand ? assets.hamburger_close : assets.hamburger_open} 
                            alt='' 
                        />
            ...
            <div>
                {/* <div className={`flex items-center cursor-pointer group relative ${expand ? 'gap-I text-white/80 text-sm p-2.5 border border-primary rounded-lg hover:bg-white/10 cursor-pointer' : 'h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg'}`}>
